...
elseif ($_REQUEST['act'] == 'edit_tracking_sn')
{
    ...
    // 更新订单中的运单号及发货时间
    // $sql = "UPDATE ".$ecs->table('order_info')." SET tracking_sn='$tracking_sn',".  " shipping_time=CURRENT_TIMESTAMP() WHERE order_id=$order_id AND tracking_sn<>'$tracking_sn'";
    $sql = "UPDATE ".$ecs->table('order_info')." SET tracking_sn='$tracking_sn',".
        " shipping_time={$_SERVER['REQUEST_TIME']} WHERE order_id=$order_id AND tracking_sn<>'$tracking_sn'";
    if($db->query($sql)) {
        // 将发货信息同步到第三方平台
...
$db->query($sql_update);
    ...
    if (!isset($_REQUEST['h'])) {
        ...
        $smarty->assign('identity_card','441224199012215128');
        ...
        if (in_array($order['region_id'],array(110000,540000,650000))) {
            $smarty->assign('region_id',$order['region_id']);
        }
...
elseif ($_REQUEST['act'] == 'get_shipping_list')
{
    ...
    if ($pay_id==32) {
        array_push($shipping_list,array('shipping_id'=>24,'shipping_name'=>'上门自提'));
    }
