...
//修改库存模板
function modStock(rec_id, value, behave, myself) {
  var elementName = myself.getAttribute('name');
  if (behave == 'get_input') {
    myself.parentNode.innerHTML = '<input type="text" onblur="modStock(' + rec_id + ',' + 'this.value' + ',' + "'modify_quantity'" + ',this' + ')" value="' + value +'" name="'+elementName+ '"/>';
  }
  else if (behave == 'modify_quantity') {
    Ajax.call('storage.php?act=modify_stock', 'rec_id=' + rec_id + '&value=' +value+'&element='+elementName, modStockRes, 'GET', 'JSON');
  }
}

function modStockRes(res) {
  var p_tag = document.getElementById('msg').getElementsByTagName('p');
  if (res.code) {
    document.getElementById(res.elements+'_'+res.rec_id).innerHTML = '<label ondblclick="modStock('+res.rec_id+",'" + res.value + "','get_input',this)" + '" name="'+res.elements+'">' + res.value + '</label>';
