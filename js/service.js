...
function quickCall(user_id,ext,manner){
  if (user_id && ext) {
    var number = '';
    var i;
    if (manner == 'contact_tel') {
      number = $("#contact_tel").find("option:selected").text();
      i = $("#contact_tel").find("option:selected").val();
    }else{
      number = $("#contact_mobile").find("option:selected").text();
      i = $("#contact_mobile").find("option:selected").val();
    }
    Ajax.call('service.php?act=quick_call','user_id='+user_id + '&ext='+ext+'&manner='+manner+'&f=1&i='+i+'&number='+number,quickCallResp,'GET','TEXT');
  }
}
