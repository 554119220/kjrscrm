...
function addNetworkBlacklist(obj)
{
  ...
  var address = obj.elements['address'].value;
  ...
  if(user_name != '')
    Ajax.call('users.php?act=putin_network_blacklist','user_name='+user_name+'&item_type='+item_type+'&number='+number+'&type_id='+type_id+'&address='+address,addNetworkBlacklistResp,'GET','JSON');
  else{
...
function advanceBatch(obj)
{
  ...
  if (!ser_startTime && !buy_startTime && !add_startTime && !max_service_time) {
    ...
    Ajax.call('users.php?act=advance_batch&','from_admin='+from_admin+'&to_admin='+to_admin+'&ser_startTime='+ser_startTime+'&ser_endTime='+ser_endTime+'&buy_startTime='+buy_startTime+'&buy_endTime='+buy_endTime+'&add_startTime='+add_startTime+'&add_endTime='+add_endTime+'&customer_type='+customer_type+'&max_service_time='+max_service_time+'&adv_number='+adv_number+'&from_customer_type='+from_customer_type,showMsg,'POST','JSON');  
  }else return false;
}
...
function searchReferrer(obj) {
  ...
  if (info = info.replace(/^\s*|\s*$/, '')) { // && info != defaultValue)
    ...
    if (document.forms['theForms'].elements['parent_id']) {
      var slt = document.forms['theForms'].elements['parent_id'];
      slt.innerHTML = '<option value="0">请先查找</option>';
...
//一键分类顾客
function onekeyClassifyUser(obj){
  if (obj.elements['class_admin_id'].value) {
    var r = confirm('你确认要一键分类顾客');
    if (r) {
      Ajax.call('users.php?act=onekey_classify_user','admin_id='+obj.elements['class_admin_id'].value,showMsg,'GET','JSON');
    }else return;
  }else return;
}
