...
elseif($_REQUEST['act'] == 'user_combine'){
    if(empty($_REQUEST['behave'] )){
        ...
        if($original_user_id && $to_user_id){
            ...
            //默认的联系方式加入联系表
            $sql_select = 'SELECT home_phone tel,mobile_phone mobile FROM '.$GLOBALS['ecs']->table('users').$where;
            $contact = $GLOBALS['db']->getAll($sql_select);
            if ($contact) {
                $sql = 'INSERT IGNORE INTO '.$GLOBALS['ecs']->table('user_contact')."(contact_name,contact_value,user_id)VALUES('%s','%s',%d)";
                foreach ($contact as $v) {
                    foreach ($v as $k=>$r) {
                        if ($r) {
                            $GLOBALS['db']->query(sprintf($sql,$k,$r,$to_user_id));
                        }
                    }
                }
            }
...
elseif($_REQUEST['act'] == 'putin_network_blacklist'){
    ...
    $address   = trim(mysql_real_escape_string($_REQUEST['address']));
    ...
    if(empty($user_name) && empty($mobile)){
        ...
        if($GLOBALS['db']->getOne($sql_select)){
            $res = crm_msg('已经存在相同的姓名');
        }else{
            $sql_insert = 'INSERT INTO '.$GLOBALS['ecs']->table('network_blacklist').
                '(user_name,mobile,type_id,address)VALUES('."'$user_name','$mobile',$type_id,'$address')";
            $code = $GLOBALS['db']->query($sql_insert);
            if($code){
                $res = crm_msg('添加成功',$code);
            }else{
                $res = crm_msg('添加黑名单失败');
            }
        }
...
//一键分类顾客
elseif($_REQUEST['act'] == 'onekey_classify_user'){
    $admin_id = intval($_REQUEST['admin_id']);
    if ($admin_id) {
        ...
        if ($r) {
            $where = " WHERE u.admin_id=$admin_id and u.customer_type=24 AND u.user_id=o.user_id AND o.final_amount>0"; 
            //添加时间 - 最后订单时间>2年 => 优质顾客
            $sql_append ='UPDATE '.$GLOBALS['ecs']->table('users').' u,'.$GLOBALS['ecs']->table('order_info')." o SET u.customer_type=%d $where ";
            $sql = $sql_append."AND o.add_time>=".strtotime(date('Y-01-01'))
                .' AND u.add_time<'.strtotime(date('Y-01-01',strtotime("-1 year")));
            $sql = sprintf($sql,1);
            $r = $GLOBALS['db']->query($sql);
            if ($r) {
                //凡两次购买以上的转入一般顾客
                for ($i = 2013; $i <= 2015; $i++) {
                    for ($j = 1; $j <= 12; $j++) {
                        ...
                        $m = $j<10 ?"0$j":$j;
                        $sql = $sql_append." AND o.add_time>=".strtotime(date("$i-$m-05")).' AND u.add_time<'.strtotime(date("$i-$m-01"));
                        $sql = sprintf($sql,2);
                        $GLOBALS['db']->query($sql);
                    }
                }
                //添加时间 < 最后订单时间 10天 => 一般顾客 至少两次购买
                //$sql = $sql_append." AND (o.add_time-u.add_time)/86400>=10 GROUP BY u.user_id";
                //$sql = sprintf($sql,2);
                //$r = $GLOBALS['db']->query($sql);
                if ($r) {
                    //订单金额>300或订单数量>3 => 潜力顾客
                    $sql = $sql_append." AND o.final_amount>300 AND o.order_status IN(5,1) AND o.shipping_id<>3";
                    $sql = sprintf($sql,22);
                    $r = $GLOBALS['db']->query($sql);
                    if ($r) {
                        //订单数3个以上
                        //$sql = 'SELECT u.user_id FROM '.$GLOBALS['ecs']->table('users').' u,'.$GLOBALS['ecs']->table('order_info').' o'
                        //    .$where." AND o.order_status IN(5,1) AND o.shipping_id<>3 GROUP BY u.user_id) HAVING COUNT(*)>=3";
                        //$sql = 'UPDATE '.$GLOBALS['ecs']->table('users')." SET customer_type=%d WHERE user_id IN $sql";
                        //$sql = sprintf($sql,22);
                        //$r = $GLOBALS['db']->query($sql);
                        //添加时间<半年 => 潜力顾客
                        $sql = $sql_p." AND customer_type=24 AND add_time<".strtotime("-6 month");
                        $sql = sprintf($sql,22);
                        $GLOBALS['db']->query($sql);
                        //跟进时间 - 最后订单时间 > 1年 => 随心所欲
                        $sql = $sql_append." AND u.order_time>0";
                        $sql = sprintf($sql,23);
                        $sql .= " AND TIMESTAMPDIFF(YEAR,FROM_UNIXTIME(u.service_time,'%Y-%m-%d'),FROM_UNIXTIME(u.order_time,'%Y-%m-%d'))>1";
                        $GLOBALS['db']->query($sql);
                        //其它
                        $sql = $sql_p." AND customer_type=24";
                        $sql = sprintf($sql,3);
                        $GLOBALS['db']->query($sql);
                    }
                }
