...
elseif($_REQUEST['act'] == 'quick_call'){
    ...
    if ($user_id && $ext) {
        ...
        if (!$i) {
            $field = $manner == 'contact_tel' ? 'home_phone' : 'mobile_phone';
            //$number = preg_replace('/\*+/','%',$number);
            $sql = "SELECT $field FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id=$user_id"; 
        }else{
            $contact_name = $manner == 'contact_tel' ? ' tel' : 'mobile';
            $sql = 'SELECT contact_value FROM '.$GLOBALS['ecs']->table('user_contact')." WHERE user_id=$user_id AND contact_value LIKE '$number'";
        }
